...
<div class="flex items-center justify-between mb-4">
    <p class="text-sm text-gray-400">Manage your call history or place a new call.</p>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dash-border">
                ...
                <tr class="hover:bg-gray-50 dash-hover">
                    ...
                    <td class="px-4 py-3"><span class="text-xs px-2 py-0.5 rounded-full {{ $call->status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">{{ ucfirst($call->status) }}</span></td>
                    <td class="px-4 py-3 text-gray-500 dash-text">{{ $call->created_at->format('M d, Y H:i') }}</td>
                    <td class="px-4 py-3 text-right whitespace-nowrap">
                        <a href="{{ route('agent.calls.edit', $call) }}" class="inline-flex p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-primary-600 transition-colors" title="Edit">
                ...
                @empty
                <tr><td colspan="7" class="text-center py-12 text-gray-400">No calls recorded.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
