...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
    <div class="dash-card lg:col-span-2 bg-white rounded-xl border border-gray-100 p-6">
        <div class="flex items-start justify-between mb-4">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">{{ $ticket->ticket_number }} | Created {{ $ticket->created_at->format('M d, Y H:i') }}</p>
            </div>
            <form method="POST" action="{{ route('company.tickets.status', $ticket) }}">
                @csrf @method('PUT')
        ...
        <div class="border-t border-gray-100 dash-border pt-4">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gray-50 dash-bg-subtle rounded-lg p-3">
                    ...
                    <p class="text-sm text-gray-600 dash-text">{{ $reply->message }}</p>
                    @if($reply->is_internal_note)<span class="text-xs text-amber-600 mt-1 inline-block">Internal Note</span>@endif
                </div>
                @endforeach
            </div>
            @endif
    ...
    <div class="dash-card bg-white rounded-xl border border-gray-100 p-5">
        ...
        <div class="space-y-3 text-sm">
            ...
            <div><p class="text-xs text-gray-400">Assigned To</p><p class="font-medium text-gray-900 dark:text-white">{{ $ticket->assignedTo->name ?? 'Unassigned' }}</p></div>
            <div><p class="text-xs text-gray-400">Created By</p><p class="font-medium text-gray-900 dark:text-white">{{ $ticket->createdBy->name ?? 'System' }}</p></div>
